...
def main():
    ...
    AddNewWords(*bigger_sequence) """ will unwrap the sequence and make them
                                    arguments in the call. one * for tuples; two ** for dictionaries """
    ...
    AddNewWords(english_spanish, 'Margarita', 'Spanish', *food_d) #only one * it works like a for loop (takes all the keys, no values)
    # This gets a SyntaxError
    # AddNewWords(*sequence, *food_d) --can only do one * in a call)
    sequence[1] = 'Raul'
    words = ['grapefruit', 'orange']
    AddNewWords(*(sequence + words)) # can add the sequences together if they are lists and *
