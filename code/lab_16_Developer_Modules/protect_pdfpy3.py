...
def ProtectPDF(options):
    ...
    command_process = subprocess.Popen(command_sequence, stderr=subprocess.PIPE) #call subprocess
    some_output = False
    ...
    if options.verbose and not some_output: # success!
        print ("Done.", end='')
        print ("%s is password-protected with the password=%s" % (options.output_pdf_file, options.password))
...
def CollectCommand(parser):
    ...
    if not options.password: #if don't have password defined, would give the error
