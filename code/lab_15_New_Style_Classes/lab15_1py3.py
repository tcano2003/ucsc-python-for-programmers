...
class SortedDictionary(dict): 
    ...
    allowed_attributes = 'description', #, makes it a tuple
...
def main():
    ...
    for initializer in (
        ...
        sorted_dict = SortedDictionary(initializer) #sorted dictionary
        print (" regular_dict.keys():", regular_dict.keys())#looking at the keys
        print ("  sorted_dict.keys():", sorted_dict.keys())
        print ("sorted_dict for-loop:", ', '.join([str(k) for k in sorted_dict])) #for loop sorts alphabetically
