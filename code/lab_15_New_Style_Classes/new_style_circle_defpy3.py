...
class Circle(list):
    ...
    def __init__(self, data, times):
        ...
        self.times = times # how many times to go around the circle

    def __getitem__(self, i):
        """circle[i] --> Circle.__getitem__(circle, i)."""
        l_self = len(self) # length of self, uses the len() of the list
        if i >= self.times * l_self:
            raise IndexError ("Circle object goes around %d times" % self.times)
        return list.__getitem__(self, i % l_self) #get the item of the list and push to self
    ...
    def __iter__(self):
        ...
        for i in range(self.times * len(self)):
            yield self[i] # make i a generator method. It will call __getitem__
