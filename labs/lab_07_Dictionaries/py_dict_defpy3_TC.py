...
def ListDefinitions(py_dict):
    ...
    for k, v in py_dict.items():
        defs += [(v, k)]
    defs.sort() #sorts by the first (v)
    for (v, k) in defs: 
        print (v, ':', k)
...
def RunMenu(py_dict):
    ...
    choices = {'add': CollectEntries, 'definitions': ListDefinitions,'find': FindDefinitions,
               'print': PrintEntries}
...
Word: 
Choose (a)dd, (d)efinitions, (f)ind, (p)rint (enter to quit) f
Word to find: for
for : set up looping
Word to find: range
